...
/*   Updated: 2018/07/05 17:22:25 by stestein         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		md5_main(int file_index, int v, char **argv)
{
	file_index = ++v;
	// handles input of flags preceeded by '-'
			...
			argv[v][1] == 'r' == 0 || argv[v][1] == 'p'))
	{
		...
		if ((ft_strcmp(argv[v], "-s") == 0 || ft_strcmp(argv[v], "-qs") == 0
				|| ft_strcmp(argv[v], "-sq") == 0 || ft_strcmp(argv[v],
					"-qrs") == 0 || ft_strcmp(argv[v], "-rsq") == 0 || 
				ft_strcmp(argv[v], "-rqs") == 0 || ft_strcmp(argv[v], "-sqr") == 0 ||
				ft_strcmp(argv[v], "-rs") == 0 || ft_strcmp(argv[v], "-sr") == 0 || 
	...
	int res = 0;
	if ((res = init_default(argv, v, &file_index)) == 0)
		return (0);
	return (1);
}
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2)
	{
		ft_printf("INVALID USE\n");
        ft_printf("\nstandard usage: %s HASH_TYPE -FLAGS THING_TO_HASH\n", argv[0]);
		ft_printf("\nread from stdin: echo 'msg' | %s HASH_TYPE -P
				|| echo 'msg' | %s HASH_TYPE\n", argv[0], argv[0]);
        return (1);
    }
	if (ft_strcmp(argv[v], "md5") == 0)
		int res = md5_main(file_index, v, argv);
	if (ft_strcmp(argv[v], "sha256") == 0)
		int res = sha256_main(file_index, v, argv);
	return (res);
}
